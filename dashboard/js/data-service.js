// ======================================================================
//  خدمة البيانات - المسؤولة عن جلب البيانات من أي مصدر
// ======================================================================
...
function getMockData(subject, month) {
    ...
    return {
        ...
        subjectName: subject === 'CS201' ? 'Database Systems (CS201)' : 
                    (subject === 'CS202' ? 'Data Structures (CS202)' : 'Artificial Intelligence (CS301)'),
...
async function getFirebaseData(subject, month) {
    ...
    return {
        ...
        questionsByType: questionsByType.length ? questionsByType : [
            { label:'MCQ', count:92, type:'mcq' },
            { label:'True/False', count:41, type:'true_false' },
...
async function fetchDashboardData(subject, month) {
    if (window.USE_FIREBASE && window.isFirebaseReady) {
        try {
            ...
            // في حالة الخطأ، نرجع بيانات وهمية كـ fallback
